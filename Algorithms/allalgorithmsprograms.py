...
class Utility:
    ...
    def mergeSort(list1):
        if len(list1) > 1:
            mid = len(list1) // 2  # Finding the mid of the array
            L = list1[:mid]  # Dividing the array elements
            R = list1[mid:]  # into 2 halves
...
def main():
    ...
    if num == 1:
        ...
        result = u.binarySearch(list1, 0, len(list1) - 1, key)
        if result != -1:
        ...
        num = input("Enter the word You want to search: ")
        f = input("Enter the file name: ")
        res = u.check(num, f)
        if res:
            print("' ", num, " ' Found in the file '", f, "'")
        else:
            print("' ", num, " ' Not Found in the file '", f, "'")
            print("---------- May be File not So does the word ----------")

    elif num == 13:
        num = int(input("Enter the number of element that you want to insert:"))
        list1 = [int(input()) for x in range(num)]
        u.mergeSort(list1)
        print("sorted list:",list1)

    elif num == 12:
        num = int(input("Enter the number of element's that you want to insert:"))
        list1 = list()
        print("Enter the elements:")
        for i in range(num):
        ...
        epsilon = float(1e-15)
        u.squareRoot(num, epsilon)

    elif num == 16:
        num = int(input("Press \n 1. Decimal To Binary \n 2. Binary To Decimal \n"))
        if num == 1:
            num = int(input("Enter the Number to convert as Binary:"))
            binary = u.decimalToBinary(num)
            print(num, " to binary is ", binary)
        ...
        swap = u.swapNibbles(num)
        print(num, " After Swapping to Nibbles : ", swap)
        print()
